  ...
  **/
  class open_console {
    ...
    */
    ...
    progress() {
      var {
        format: _format = this.throwIfMissing('format'),
        formatData,
        prefix = '',
        config = {}
      } = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
      ...
      */
      ...
      var multibar = new progress.MultiBar(_objectSpread2(_objectSpread2({}, {
        clearOnComplete: false,
        hideCursor: false,
        barCompleteChar: '\u2588',
        barIncompleteChar: '\u2591',
        barsize: 20,
        format: function format(options, params, payload) {
          ...
          ndata._format = _format;
          ndata = formatData(ndata); // test changes

          if (ndata._format != _format) ; //prefix
          ...
          var resp = '';
          if (used_prefix != '') resp += used_prefix; //create resp from defined format

          delete ndata.funcs;
          var pattern = extract(ndata._format);
          ndata = _objectSpread2(_objectSpread2({}, ndata), payload);
    ...
    out() {
      ...
      if (!this.config.silent) {
        ...
        if (data) {
          ...
          var edata = util.inspect(data, false, null, true);
